...
def generate_adv(model, images, labels, attack):
    ...
    if attack == Attacks.FGSM:
        ...
        adversarials = adv_attacks.get_fgsm_adv_samples(model, images, labels, 0.4, None)

    if attack == Attacks.BIM:
        print('generating BIM adversarials...')
        adversarials = adv_attacks.get_bim_adv_samples(model, images, labels, 0.4, 5, None)

    if attack == Attacks.CW:
        print('generating CW adversarials...')
        adversarials = adv_attacks.get_cw_adv_samples(model, images, labels, tensorboard_path=None)

    plt.imshow(np.squeeze(adversarials[0]))
    plt.show()
